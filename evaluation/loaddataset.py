...
class NIConfig(datasets.BuilderConfig):
    def __init__(self, *args, task_dir=None, max_num_instances_per_task=None, max_num_instances_per_eval_task=None, corruption=None, **kwargs):
...
class NaturalInstructions(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        if self.config.data_dir is None or self.config.task_dir is None:
            ...
            self.config.corruption = self.corruption
            self.config.max_num_instances_per_eval_task = self.max_num_instances_per_eval_task

        split_dir = self.config.data_dir
        task_dir = self.config.task_dir

        print(os.path.join(split_dir, "test_tasks.txt"),  task_dir, self.config.max_num_instances_per_eval_task, self.config.corruption)
    ...
    def _generate_examples(self, path=None, task_dir=None, max_num_instances_per_task=None, corruption=None, subset=None):
        """Yields examples."""
        if subset=="test":
            ...
            with open(path, encoding="utf-8") as split_f:
                for line in split_f:
                    ...
                    task_path = os.path.join(task_dir, task_name + "_"+ corruption + ".json")
